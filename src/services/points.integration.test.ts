...
run('PointsService (integration)', () => {
  ...
  afterAll(async () => {
    await prisma.pointsBalance.deleteMany({ where: { userId: { in: [USER_A, USER_B] } } });
    await prisma.user.deleteMany({ where: { id: { in: [USER_A, USER_B] } } });
    await prisma.$disconnect();
  });

  it('awards and reads a balance', async () => {
    await points.award(USER_A, 100);
    expect(await points.getBalance(USER_A)).toBe(100);
  });

  it('floors negative balances at zero', async () => {
    await points.award(USER_A, -1000);
    expect(await points.getBalance(USER_A)).toBe(0);
  });

  it('handles concurrent awards without lost updates', async () => {
    await points.award(USER_A, -1_000_000); // reset to 0
    await Promise.all(Array.from({ length: 50 }, () => points.award(USER_A, 2)));
    expect(await points.getBalance(USER_A)).toBe(100);
  });

  it('rejects overspending', async () => {
    await expect(points.spend(USER_B, 5)).rejects.toBeInstanceOf(InsufficientPointsError);
  });

  it('awardCapped stops at the cap and never reduces an over-cap balance', async () => {
    await points.award(USER_A, -1_000_000); // reset to 0
    // Accrue toward the cap.
    await points.awardCapped(USER_A, 25, 3000);
    expect(await points.getBalance(USER_A)).toBe(25);
    // Near the cap: only rises to the cap, not past it.
    await points.award(USER_A, 2975); // now 3000
    await points.awardCapped(USER_A, 25, 3000);
    expect(await points.getBalance(USER_A)).toBe(3000);
    // Already above the cap (e.g. via a gift): capped award leaves it untouched.
    await points.award(USER_A, 500); // 3500
    await points.awardCapped(USER_A, 25, 3000);
    expect(await points.getBalance(USER_A)).toBe(3500);
  });

  it('transfers atomically', async () => {
    await points.award(USER_A, -1_000_000); // reset A to 0
    await points.award(USER_A, 30);
    await points.transfer(USER_A, USER_B, 10);
    expect(await points.getBalance(USER_A)).toBe(20);
    expect(await points.getBalance(USER_B)).toBe(10);
  });
});
